...
class SSMResults:
    def __init__(self, results: pd.DataFrame, scores: pd.DataFrame, details: Dict[str, Any], call: str):
    ...
    def __str__(self):
        ...
        for _, row in self.results.iterrows():
            ...
            params = ['Elevation', 'X-Value', 'Y-Value', 'Amplitude', 'Displacement', 'Model Fit']
            ...
            for param, est, lci, uci in zip(params, estimates, lower_ci, upper_ci):
                output.append(f"{param:<{max_len}} {est:>8.3f} [{lci:>8.3f}, {uci:>8.3f}]")
