...
class Instrument:
    def __init__(self, scales: pd.DataFrame, anchors: pd.DataFrame, items: pd.DataFrame,
                 norms: Dict[str, Any], details: Dict[str, Any]
                 ):
        self.scales = scales
    ...
    def __str__(self):
        return (f"{self.details['Abbrev']}: {self.details['Name']}\n"
    ...
    def get_scales(self, items=False):
        output = [f"The {self.details['Abbrev']} contains {self.details['Scales']} circumplex scales."]
        for _, scale in self.scales.iterrows():
            output.append(f"{scale['Abbrev']}: {scale['Label']} ({scale['Angle']} degrees)")
            if items:
    ...
    def get_items(self):
        output = [f"The {self.details['Abbrev']} contains {self.details['Items']} items ({self.details['Status']}):"]
    ...
    def get_anchors(self):
        output = [f"The {self.details['Abbrev']} is rated using the following {len(self.anchors)}-point scale."]
    ...
    def get_norms(self):
        ...
        output = [f"The {self.details['Abbrev']} currently has {n_norms} normative data set(s):"]
