...
def test_ssm_analyze_with_measures(sample_data, scales, angles):
    result = ssm_analysis.ssm_analyze(sample_data, scales, angles, measures=['measure1', 'measure2'], boots=50)
    assert isinstance(result, SSMResults)
    assert len(result.results) == 2  # One row for each measure


def test_ssm_analyze_with_grouping(sample_data, scales, angles):
    result = ssm_analysis.ssm_analyze(sample_data, scales, angles, grouping='group', boots=50)
    assert isinstance(result, SSMResults)
    assert len(result.results) == 2  # One row for each group


def test_ssm_analyze_with_contrast(sample_data, scales, angles):
    result = ssm_analysis.ssm_analyze(sample_data, scales, angles, grouping='group', contrast='test', boots=50)
    assert isinstance(result, SSMResults)
...
def test_ssm_bootstrap(sample_data, scales, angles):
    ...
    result = ssm_analysis.ssm_bootstrap(
            bs_input, bs_function, np.deg2rad(angles),
            100, 0.95, 'none', True, bs_input['group']
            )
...
@pytest.mark.parametrize("amplitude, displacement, elevation", [
    (1.0, 3, 0.5),  # 3 degrees
...
def test_ssm_parameters_correctness(amplitude, displacement, elevation):
    ...
    # Extract results
    result_elevation, result_x, result_y, result_amplitude, result_displacement, result_r2 = params
