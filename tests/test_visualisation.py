...
def test_ssm_plot(mock_ssm_results):
    ...
    assert len([c for c in ax.get_children() if isinstance(c, PathCollection)]) > 0, (
...
def test_ssm_plot_circle(mock_ssm_results):
    ...
    assert len(wedges) == len(mock_ssm_results.results), (
    ...
    assert len(scatter_points[0].get_offsets()) == len(mock_ssm_results.results), (
...
def test_ssm_plot_profile(mock_ssm_results):
    ...
    for ax in axes:
        ...
        assert any(len(line.get_xdata()) > 2 for line in lines), (
